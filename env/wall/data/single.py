...
class DotDataset:
    ...
    def render_location(self, locations: torch.Tensor):
        x = torch.linspace(0, self.config.img_size - 1, steps=self.config.img_size, device=self.device)
        y = torch.linspace(0, self.config.img_size - 1, steps=self.config.img_size, device=self.device)
    ...
    def generate_state(self, wall_locs=None, door_locs=None, size=None):
        ...
        location = (
            torch.rand(size=(size, 2), device=self.device) * effective_range + self.padding
        )
    ...
    def generate_transitions(self, location, actions, bias_angle):
        ...
        for i in range(actions.shape[1]):
            ...
            for _ in range(self.config.repeat_actions):
                current_location = self.generate_transition(current_location, actions[:, i])
    ...
    def generate_actions(self, n_steps, bias_angle=None):
        ...
        von_mises_dist = torch.distributions.VonMises(concentration=concentration, loc=0.0)
        angles = [bias]
        ...
        truncated_normal_dist = truncnorm(a, b, loc=self.config.action_step_mean, scale=self.config.action_step_std)
